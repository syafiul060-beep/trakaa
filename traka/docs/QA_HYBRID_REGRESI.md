...
- **Cari travel (daftar driver):** Firebase Analytics `passenger_active_drivers_source` (`source`: `geo_match` | `api_list` | `firestore`, `result_count`, opsional `reason`). Jika hybrid on tetapi jatuh ke Firestore → breadcrumb Crashlytics `[DriverHybrid] passenger.activeDrivers.firestore_fallback …`.
