...
- **Cari travel (daftar driver):** Firebase Analytics `passenger_active_drivers_source` (`source`, `result_count`, `reason`, `fs_cap=1` jika plafon Firestore tercapai). Fallback Firestore memakai **limit 400** dokumen `siap_kerja` (subset acak jika lebih banyak driver aktif). Breadcrumb: `firestore_fallback`, `firestore_cap_hit`.
